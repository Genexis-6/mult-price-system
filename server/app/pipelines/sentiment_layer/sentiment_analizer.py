...
from app.store.schemas import ProductSentimentSchemas
import os
from app.utils.logger import get_logger
from app.store import db_session_manager
from transformers import pipeline, logging as hf_logging
from app.store.queries import JijiQueries, JumiaQueries, KongaQueries
...
class SentimentAnalyzer:
    ...
    async def score_sentiment(self, QueryClass):
    ...
    def score(self, product: ProductSentimentSchemas, source: str = "unknown") -> ProductSentimentSchemas:
    ...
    async def run_sentiment(self):
        logger.info("run_sentiment started for all sources")
        start = time.perf_counter()

        tasks = [
            asyncio.create_task(self.score_sentiment(JumiaQueries)),
            asyncio.create_task(self.score_sentiment(KongaQueries)),
            asyncio.create_task(self.score_sentiment(JijiQueries)),
        ]
