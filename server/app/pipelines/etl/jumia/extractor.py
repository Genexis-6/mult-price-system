...
async def fetch(session: aiohttp.ClientSession, url: str, render_js: bool = False) -> Optional[str]:
    """Fetch a URL through ScraperAPI with exponential-backoff retries."""
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            async with sem:
                async with session.get(url=url, timeout=REQUEST_TIME_OUT, headers=HEADERS) as resp:
                    if resp.status == 429:
...
async def _scrape_jumia_async(query: str, pages: int) -> List[dict]:
    async with aiohttp.ClientSession() as session:
        ...
        catalog_htmls = await asyncio.gather(
            *[fetch(session, _build_catalog_url(query, p)) for p in range(1, pages + 1)]
        )
